(function () {
  ...
  const FEATURES = {
    ...
    website: {
      ...
      lead: 'AKILI plans the investigation first, then chains SSL, headers, ports, exposed files, CVE lookups, and crawler paths — like a senior analyst, not a checkbox scanner.',
      bullets: [
        'Planning-first agent picks the right tools for your URL',
        'Technology + CVE chaining when versions are detected',
        'Live streaming log so you see each step',
        'Legitimacy assessment with page context',
      ],
      icon: 'globe',
    ...
    vulnerability: {
      ...
      lead: 'Hunt misconfigs, risky headers, exposed admin paths, and common web flaws with AI-guided follow-ups.',
      bullets: ['CORS, CSRF, and injection pattern checks', 'Exposed config and backup probing', 'CVE mapping on detected stack'],
      icon: 'bug',
      accent: 'var(--mod-vuln)',
    },
    subdomains: {
